/**
 * WebP responsivos para fotos de Bethel.
 * Fuentes: .jpg, .jpeg o .png en public/images/bethel/ (p. ej. bethel-1.jpg)
 * Salida: bethel-1-480.webp, bethel-1-720.webp, bethel-1-1080.webp
 * Uso: node scripts/optimize-bethel-images.mjs
 */
...
const SRC_DIR = path.join(ROOT, "public", "images", "bethel");
...
async function main() {
  ...
  for (const e of entries) {
    ...
    if (!m) continue;
    const base = m[1];
    if (new RegExp(`-(${WIDTHS.join("|")})$`).test(base)) continue;
    slugs.add(base);
  }

  if (slugs.size === 0) {
    console.warn("No hay fuentes en public/images/bethel");
    return;
  }
